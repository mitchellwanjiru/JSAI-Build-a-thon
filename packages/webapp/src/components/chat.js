...
export class ChatInterface extends LitElement {
  ...
  //         />
  //         <button @click=${this._sendMessage} ?disabled=${this.isSendDisabled}>
  //           Send
  //         </button>
  //       </div>
  //     </div>
  ...
  // Getter to determine if the send button should be disabled
  get isSendDisabled() {
  ...
  async _sendMessage() {
    ...
    try {
      ...
      this.messages = [
        ...
        { 
          role: 'assistant', 
          content: aiResponse.reply,         // Only the reply text
          sources: aiResponse.sources || []  // Optionally add sources for RAG display, if present
        }
      ];
    } catch (error) {
